...
class HeaterAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    this.normalAsync(Characteristic.Manufacturer, "Solaire");
    this.normalAsync(Characteristic.Model, "R4");
    this.normalAsync(Characteristic.SerialNumber, this.deviceId);
    this.normalAsync(Characteristic.FirmwareRevision, this.deviceConfig.firmwareVersion || "1.0.0");
    this.normalAsync(Characteristic.Name, this.deviceConfig.name || this.homebridgeAccessory.displayName);
...
if (statusMap.code === 'set_water_temp' || statusMap.code === 'temp_set_f') {
  ...
  // Initialize cached value (but don’t call updateValue here)
  ...
  heatChar
    ...
    .on('set', (value, callback) => {
      ...
      this.platform.tuyaOpenApi.sendCommand(this.deviceId, param)
        ...
        .catch(err => {
          this.log.error(
            '[SET][%s] HeatingThresholdTemperature Error: %s',
