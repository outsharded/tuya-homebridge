  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (statusMap.code === 'set_water_temp' || statusMap.code === 'temp_set_f') {
        ...
        // 1️⃣ get the characteristic first
        const targetTempChar = this.service.getCharacteristic(Characteristic.TargetTemperature);
      
        // 2️⃣ set props immediately
        ...
        // 3️⃣ set initial value
        this.setCachedState(Characteristic.TargetTemperature, statusMap.value);
        targetTempChar.updateValue(statusMap.value);
      
        // 4️⃣ attach get/set handlers
        targetTempChar
          .on('get', callback => {
            callback(null, this.getCachedState(Characteristic.TargetTemperature));
          })
          .on('set', (value, callback) => {
            const param = this.getSendParam(Characteristic.TargetTemperature, value);
            this.platform.tuyaOpenApi.sendCommand(this.deviceId, param)
              .then(() => {
                this.setCachedState(Characteristic.TargetTemperature, value);
                callback();
              })
              .catch(err => {
                this.log.error('[SET][%s] TargetTemperature Error: %s', this.homebridgeAccessory.displayName, err);
