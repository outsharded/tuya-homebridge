  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (statusMap.code === 'set_water_temp' || statusMap.code === 'temp_set_f') {
        ...
        const tempRange = this.temp_set_range || (statusMap.code === 'set_water_temp' ? { min: 30, max: 70 } : { min: 86, max: 158 });

        this.normalAsync(Characteristic.TargetTemperature, statusMap.value, {
